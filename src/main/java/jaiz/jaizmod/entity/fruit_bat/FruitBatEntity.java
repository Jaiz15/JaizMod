...
public class FruitBatEntity extends BatEntity {
    ...
    public void tick() {
        if (!this.getWorld().isClient && this.isAlive() && --this.poopTime <= 0) {
            GuanoEntity poop = ModEntities.GUANO.create(this.getWorld());
            if (poop != null) {
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (itemStack.isOf(Items.SWEET_BERRIES) && this.isRoosting()) {
            this.playSound(SoundEvents.ENTITY_GENERIC_EAT, 0.75f, (this.random.nextFloat() - this.random.nextFloat()) * 0.2f + 1.0f);
            if(!player.isCreative()){
            itemStack.decrement(1);}
            GuanoEntity poop = ModEntities.GUANO.create(this.getWorld());
            if (poop != null && this.random.nextInt(4) == 1) {
